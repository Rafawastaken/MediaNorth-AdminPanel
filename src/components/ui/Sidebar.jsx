...
// -------------------------------------------------------------------
// Sidebar colapsável: quando `collapsed` é true mostra só ícones.
  ...
  return (
      <div className="text-[15px] leading-tight text-gray-200">
        <button
            ...
            className={`flex w-full items-center gap-3 px-4 py-3 hover:bg-white/5 ${collapsed ?
                'justify-center' :
                ''}`}
        >
          <Icon size={22}/>
          {!collapsed && (
              <>
                <span
                    className="flex-1 text-left font-medium tracking-wide">{label}</span>
...
const Item = ({icon: Icon, to, children, collapsed}) => (
    <Link
        ...
        className={`flex items-center gap-3 rounded px-3 py-2 hover:bg-white/5 transition ${collapsed ?
            'justify-center' :
            ''}`}
    >
...
const Sidebar = ({collapsed = false}) => {
  ...
  return (
      <aside
          className={`flex h-screen flex-col bg-[var(--dark-blue)] transition-all duration-300 ${collapsed ?
              'w-20' :
              'w-72'}`}
      >
        {/* Logo / título */}
        <div
            className={`flex items-center gap-3 border-b border-white/10 px-4 ${collapsed ?
                'h-20 justify-center' :
                'h-24'}`}>
          <Tv size={collapsed ? 28 : 32}
          ...
          {!collapsed && (
              <div>
                <h1 className="text-xl font-semibold text-white leading-none">Media&nbsp;North</h1>
        ...
        <nav className="flex-1 overflow-y-auto py-6">
          <Section icon={LayoutDashboard} label="Dashboard"
        ...
        {collapsed ? (
            <div
                className="flex h-16 items-center justify-center border-t border-white/10">
              <div
                  className="grid h-10 w-10 place-items-center rounded-full bg-slate-600/40">
                <User size={18}/>
              </div>
            </div>
        ) : (
